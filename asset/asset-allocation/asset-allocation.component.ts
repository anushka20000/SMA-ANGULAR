...
export class AssetAllocationComponent implements OnInit,OnDestroy{
  ...
  public viewNewAssetModalStyle:string='none';
  ...
  constructor(private http: HttpClient, private renderer: Renderer2,
              ...
              private modelService: DemoServiceService,
  ...
  private loadGrid() {
    ...
    let url = `${environment.apiUrl}/news-letter-grid?dT=true`;
    //alert(url);
      ...
      columns: [
...

        {
          ...
          render: function (data: any, type: any, full: any) {
            ...
            return '<button  title="Edit '+that.commonEntityName+'" action="'+that.commonEntityNameAttribute+'_edit" data-id="' + full.id + '"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">' +
                '<span class="svg-icon svg-icon-3">' +
                '<img width="16" height="16"  action="'+that.commonEntityName.replace(/\s/g, '_').toLowerCase()+'_edit" data-id="' + full.id + '" ' +
                'src="./assets/media/icons/schoolnet/edit.svg"></span></button>' +

                '<button  title="Delete '+that.commonEntityName+'" action="'+that.commonEntityNameAttribute+'_delete" data-id="' + full.id + '"  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">' +
                '<span class="svg-icon svg-icon-3">' +
                '<img width="16" height="16" action="'+that.commonEntityNameAttribute+'_delete" data-id="' + full.id + '" ' +
                'src="./assets/media/icons/schoolnet/delete.svg"></span></button>';
  ...
  openModal(modalId:number)
  {
    if(modalId==1)
    {
      this.viewNewAssetModalStyle='block';
    }

  }
  closePopup()
  {
    this.viewNewAssetModalStyle='none';
  }
